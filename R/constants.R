...
sig_cnt_summary = function(data, sig_cnt_col='sig_cnt'){
  sums = data %>%
    ...
    summarise(means=mean(get(sig_cnt_col), na.rm=TRUE), 
              ...
              props=sum(get(sig_cnt_col)>0, na.rm = T)/sum(get(sig_cnt_col)>=0, na.rm=T))
  return(sums)
}
...
match_gene_subset_by_caf = function(ref_data, gene_subset, freq_col='caf', id_col='gene_id', sig_col='sig_cnt', 
  ...
  set.seed(seed)
  sim_data = replicate(sim_times, match_one_set_by_caf(remain_data, bin_sum), simplify=FALSE)
...
match_one_set_by_caf = function(remain_data, bin_sum, sig_col='sig_cnt'){
  n = nrow(bin_sum)
  ids = vector()
  pend = 0
  for(i in n:1){
