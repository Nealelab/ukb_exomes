...
var_info  = load_ukb_file('var_info.txt.bgz')
...
var_sum = rbind(var_mis, var_non_mis) %>%
  mutate(annotation=factor(annotation, levels = annotation_types),
         polyphen2=factor(polyphen2, level = c('probably_damaging', 'possibly_damaging', 'benign','')))
...
plt_3a = var_sum %>%
  ...
  geom_text(data = var_sum, aes(x = annotation, y = prop, label = polyphen2), size = 1.5, vjust = -3, hjust = -0.00001, position = position_dodge(1) ) +
...
plt_3b = blosum_info %>%
  na.omit() %>%
  filter(cnt>=100) %>% 
  ggplot + aes(x = score, y = prop, size = cnt) + 
...
pext = var_info %>%
  filter(tx_annotation_csq == most_severe_consequence & 
           ...
           lof == 'HC') %>% 
  mutate(mean_prop_bin=factor(get_mean_prop_interval(mean_proportion), 
                              levels = c('[0, 0.2]','(0.2, 0.8]','(0.8, 1]')))%>% 
...
plt_3d = gene_sum %>%
  ...
  ggplot + aes(x=oe_lof_upper_decile, y=prop, ymin=prop-sd, ymax=prop+sd, color=annotation, fill=annotation) + 
  geom_pointrange(stat="identity", position=position_dodge(width=0.4), size=0.4) +
  labs(y = 'Proportion', x = 'oe_lof_upper_decile')  + 
  scale_y_continuous(label=label_percent(accuracy=1)) +
  annotation_color_scale + annotation_fill_scale  + 
  theme_classic() + themes
...
ggarrange(plt_3a, plt_3b, plt_3c, plt_3d, nrow=2, 
          label.args = list(gp=gpar(font=2, cex=1.2), vjust=1))
dev.off()
