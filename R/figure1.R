...
figure1 = function(save_plot = F, output_path){
  figure1_cnt = data.frame(
    type = rep(c('Variants', 'Genes', 'Phenotypes'), 2),
  ...
  figure1b = save_count_barplot_figure(cnt_data = figure1_cnt, cnt_type = 'Genes',  save_plot = T, output_path = paste0(output, 'figure1b_gene_cnt.png'))
  ...
  figure1e = save_count_by_freq_figure(cnt_data = gene_cnt_by_freq, type = 'Genes', save_plot = T, output_path = paste0(output, 'figure1e_gene_cnt_by_CAF.png'))

  figure1_top =  egg::ggarrange(figure1a, figure1b, figure1c, labels = c('(A) Variants', '(B) Genes', '(C) Phenotypes'), ncol = 3,
                                label.args = list(gp=gpar(font = 2, cex = 0.75), vjust = 1.5, hjust = 0))
  figure1_bottom = egg::ggarrange(figure1d, figure1e, labels = c('(D) Variants by AF Interval', '(E) Genes by CAF Interval'), ncol = 2,
                                  label.args = list(gp=gpar(font = 2, cex = 0.75), vjust = 1.5))
  figure = ggpubr::ggarrange(figure1_top, figure1_bottom, nrow = 2)

  if(save_plot){
    png(paste0(output_path, 'figure1.png'), height = 6, width = 9, units = 'in', res = 300)
    print(figure)
    dev.off()
  }
  return(figure)
...
figure1(save_plot = T, output_path = output)