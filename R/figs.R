...
# --------QCed Lambda GC----------
lambda_gene_full  = load_ukb_file('lambda_gene_full300k_filtered_coverage.txt.bgz')
lambda_gene_annt  = load_ukb_file('lambda_gene_annt300k_filtered_coverage.txt.bgz')
lambda_gene_af  = load_ukb_file('lambda_gene_af300k_filtered_coverage.txt.bgz')
lambda_gene_af_annt  = load_ukb_file('lambda_gene_af_annt300k_filtered.txt.bgz')

lambda_var_full  = load_ukb_file('lambda_var_full300k_filtered.txt.bgz')
lambda_var_annt  = load_ukb_file('lambda_var_annt300k_filtered.txt.bgz')
lambda_var_af  = load_ukb_file('lambda_var_af300k_filtered.txt.bgz')
lambda_var_af_annt  = load_ukb_file('lambda_var_af_annt300k_filtered.txt.bgz')

lambda_by_gene  = load_ukb_file('lambda_by_gene300k_filtered_coverage.txt.bgz')
...
p_lambda_gene_full_before = lambda_gene_full_before %>%
  ...
  geom_point(alpha = 0.5, size = 2) + ylim(0, 2) + theme_classic() +
  labs(y = 'Lambda GC', x = '') +
...
egg::ggarrange(p_lambda_gene_full_before, p_lambda_gene_full_after,  labels = c('Before', 'After'))
...
lambda_by_gene %>%
  ...
  facet_grid(result_type~annotation, scale='free', labeller = label_type) +
  geom_text(data = gene_cnt2, aes(label = paste('Genes with lambda>2: ',as.character(cnt)), x = 1.5, y = 2.5,  face = 'bold', color = annotation, group = NULL), size = 3)
