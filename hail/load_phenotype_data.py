...
def read_covariate_data(pre_phesant_data_path, temp_ht_path=None):
    if hl.hadoop_exists(f'{temp_ht_path}/_SUCCESS'):
...
def load_custom_data():
    ...
    # Pfizer
    ht = hl.import_table(f'{custom_data_folder}/Pfizer_custom_phenotype_450K_04152021.txt', impute=True, delimiter='\t', missing="NA", key='f.eid', min_partitions=8)
    ...
    pfizer_meta = hl.import_table(f'{custom_data_folder}/Pfizer_custom_phenotypes_04152021_edit.txt', delimiter='\t', impute=True)
    pfizer_meta = pfizer_meta.annotate(coding=NULL_STR_KEY,
                                       pheno_sex=hl.if_else(pfizer_meta.pheno_sex.endswith('male'), pfizer_meta.pheno_sex + 's', pfizer_meta.pheno_sex),
                                       modifier='pfizer'
                                       ).select(*PHENO_KEY_FIELDS, *PHENO_DESCRIPTION_FIELDS).key_by('trait_type', 'phenocode')
...
def read_random_phenos():
    random_mt = hl.import_matrix_table('gs://ukbb-pharma-exome-analysis/300k/random_phenos2.tsv.bgz',
...
def main(args):
    ...
    if args.load_data:
        # temp_directory = f'{temp_bucket}/{CURRENT_TRANCHE}'
        # load_icd_data(get_pre_phesant_data_path(), icd_codings_ht_path, temp_directory).write(get_ukb_pheno_mt_path('icd', 'full'), args.overwrite)
        # read_covariate_data(get_pre_phesant_data_path(), f'{temp_directory}/pre_phesant.ht'
        #                     ).write(get_ukb_covariates_ht_path(), args.overwrite)
    ...
    if args.add_covid_wave:
        # hail load_phenotype_data.py --add_covid_wave 04162021 --overwrite
        # python saige_pan_ancestry.py --phenos .*COVID.*04162021.*
        ...
        cov_ht = hl.read_table(get_ukb_covariates_ht_path())
        mt = combine_pheno_files_multi_sex({'custom': mt.key_rows_by(userId=mt.eid)}, cov_ht)
    ...
    ht = ht.annotate(keep=(exclusion_ht[ht.key]['status.remove'] != 'remove') | (
                ht.phenocode == 'COVID19') | ht.phenocode.startswith('random_'))
