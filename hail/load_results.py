...
def main(args):
    ...
    for i in range(chunk_size):
        ...
        outer_mts.append(mt.checkpoint(f'{temp_bucket}/gene_output_{i}.mt', overwrite=True))
    mt = outer_mts[0]
    ...
    for directory in all_phenos:
        ...
        ht = ht.key_by(locus=hl.parse_locus(locus_alleles[0]), alleles=locus_alleles[1].split('/'),
                       pheno=pheno, coding=coding).distinct()
        ...
        ht.checkpoint(output_ht_path, overwrite=args.overwrite, _read_if_exists=not args.overwrite)
    ...
    # ht2 = ht.transmute(pheno_key=hl.struct(pheno=ht.phenotype, coding=ht.coding))
    # phenos = ht2.aggregate(hl.agg.collect_as_set(ht2.pheno_key), _localize=False)
    # ht = ht2.annotate_globals(all_phenos=hl.sorted(hl.array(phenos))).collect_by_key()
    # ht = ht.annotate(values=hl.sorted(ht.values, key=lambda x: x.pheno_key))
    # ht._unlocalize_entries('values', 'all_phenos', ['pheno', 'coding'])
